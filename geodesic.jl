...
println("\t Non-inplace")
fit = @time curve_fit(model, jacobian_model, xdata, ydata, p0; maxIter=1000, show_trace = true)
fit.converged

println("\t Geodesic")
fit_geo = @time curve_fit(model, jacobian_model, xdata, ydata, p0; maxIter=1000, avv! = manual_avv!,lambda=0, min_step_quality = 0, show_trace = true)
fit_geo.converged


println("\t Geodesic - auto avv!")
fit_geo_auto = @time curve_fit(model, jacobian_model, xdata, ydata, p0; maxIter=1000, avv! = auto_avv!,lambda=0, min_step_quality = 0, show_trace = true)
fit_geo_auto.converged


maximum(abs.(fit.param-[1.0, 2.0])) < 1e-1
maximum(abs.(fit.param-fit_geo.param)) < 1e-6
maximum(abs.(fit.param-fit_geo_auto.param)) < 1e-6
...
fit_geo_auto_wt.converged


maximum(abs.(fit_wt.param-[1.0, 2.0])) < 1e-1
maximum(abs.(fit_wt.param-fit_geo_wt.param)) < 1e-6
maximum(abs.(fit_wt.param-fit_geo_auto_wt.param)) < 1e-6

# %% let me try a model approach
...
function mod2(beta, p)
    psf.objvec(beta, wh, xmat, geotargets)
end

function jacobian_mod2(beta, p)
    J = Array{Float64}(undef, length(beta), length(p))
    @. J[:,1] = exp(-x*p[2])     #dmodel/dp[1]
    @. @views J[:,2] = -x*p[1]*J[:,1]
    J
end

function jfvec2(beta, p)
    ForwardDiff.jacobian(x -> fvec(x), beta)
end

#Generating the "automatic" avv
function mod2_inplace(out, p)
    out = psf.objvec(beta, wh, xmat, geotargets)
end

p0 = [1.0]
hessians = Array{Float64}(undef, length(geotargets)*length(p0), length(p0))
h! = make_hessian(mod2_inplace, geotargets, p0)
auto_avv! = Avv(h!, length(p0), length(geotargets))

curve_fit(mod2, jfvec2, vec(geotargets), vec(geotargets), p0; maxIter=10, avv! = auto_avv!,lambda=0, min_step_quality = 0, show_trace = true)
